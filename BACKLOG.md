...
| Per-strategy allocated capital for equity pause trigger | Both mv-afts (clientId=1) and mv-opt-one (clientId=2) run on the same IBKR paper account. Both call `get_account_equity()` and pause if NLV drops 20%. A loss in one strategy can trip the other's pause trigger. Fix: add `allocated_capital: Decimal` to `IBKRCfg` or strategy config; pause trigger compares strategy P&L against allocated capital, not total NLV. **Required before live.** Paper validation can proceed without it — false pauses are safe. |
